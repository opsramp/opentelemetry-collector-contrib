...
new events.
- `exclude_namespaces` (default = `[]`): An array of namespaces to exclude from event collection.
  - When `namespaces` is empty (watch all): events from excluded namespaces are dropped per-event.
  - When `namespaces` is non-empty: excluded namespaces are removed from the watch list (set difference), reducing K8s API watch connections.
  - Empty or nil means no exclusions (backward compatible).
